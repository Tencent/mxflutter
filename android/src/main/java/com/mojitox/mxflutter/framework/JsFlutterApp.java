...
public class JsFlutterApp {
        ...
        jsFlutterAppChannel.setMethodCallHandler(new MethodChannel.MethodCallHandler() {
            ...
            public void onMethodCall(@NonNull MethodCall methodCall, @NonNull MethodChannel.Result result) {
                if (methodCall.method.equals(MethodChannelConstant.CALL_JS)) {
                    if (!isJsAppRun) {
                        //  MXJSC-Support：v0.9.0 nativeCall 参数改为String
                        MxLog.w(TAG, "jsApp is not running" + methodCall.arguments);
    ...
    private void onMainJsLoadComplete(long jsLoadStartTime) {
        MxLog.d(TAG, "on main js load success");
        isJsAppRun = true;
    ...
    private void callJSInitProfileInfo() {
        ...
        String argsStr = new JSONObject(args).toString();
        MethodCall methodCall = new MethodCall(MethodChannelConstant.NATIVE_CALL_METHOD, argsStr);
        if (!isJsAppRun) {
