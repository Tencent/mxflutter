...
+ (void)preloadJSEngine:(void (^)(BOOL success, NSString *errorMsg))completion {
    ...
    // 因为是预加载方法，因此从默认路径读取
    NSString *mainJS = [[MXCommonUtil defaultJSAppUpdatePath] stringByAppendingPathComponent:@"main/main.js"];
...
- (NSString *)calcRequireJSAbsolutePath:(NSString *)filePath {
    ...
    if(absolutePath.length == 0){
        MXFLogError(@"requrie can not found file: %@",filePath);
    }
    return absolutePath;
}
...
- (void)callJSCallbackFunction:(NSString *)callbackId param:(id)param {
    ...
    if (callback) {
        // MX Modified begin JSC：参数个数要一致,MXC JS代码会检查
        param = param?:[NSNull new];
