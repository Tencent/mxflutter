...
- (void)setupChannel {
    ...
    [self.jsFlutterAppChannel setMethodCallHandler:^(FlutterMethodCall *_Nonnull call, FlutterResult _Nonnull flutterResult) {
        ...
        if ([call.method isEqualToString:@"callJS"]) {
            ...
            MXFLogDebug(@"MXJSFlutter : jsFlutterAppChannel callJS:%@",call.arguments);
            if (!strongSelf.isJSAPPRun) {
                MXFLogDebug(@"MXJSFlutter : jsFlutterAppChannel callJS:%@ JSAPP not running",call.arguments);
...
- (void)setupFFIImpl {
    [MXFFI sharedInstance].delegate = [MXFFIImpl sharedInstance];
}
...
- (void)callJSInitProfileInfo {
    ...
    //  MXJSC-Support： nativeCall 参数改为String
...
- (NSString *)searchEntryJS {
    ...
    NSString *pkgMainJS = [[MXCommonUtil defaultJSAppUpdatePath] stringByAppendingPathComponent:@"main/main.js"];
