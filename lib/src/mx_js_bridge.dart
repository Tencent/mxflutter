...
class MXJSBridge {
  ...
  Future<String> mxJSBridgeInvokeMirrorObjWithCallback(args) async {
    ...
    MXMirror.getInstance().invokeFunctionWithCallback(args, (result) {
      ...
      if (result != null &&
        ...
        // 要求返回值为string类型
        returnJsonStr = _encodeInvokeReturnValue(result);
      }
  ...
  String _encodeInvokeReturnValue(dynamic returnValue) {
    return json.encode(returnValue, toEncodable: (dynamic object) {
      // TODO: 此处会要求返回值为string类型，否则会因为encode异常。先返回类型。
      MXJSLog.log(
          "InvokeMirrorObjWithCallback: returnValue encode json error : className: $object");
      return {"className": object.runtimeType.toString()};
    });
  }
  ...
  /// Flutter 和 JS 互调顶层通用通道。
  _setupCommonBasicChannel() {
    _commonBasicChannel = BasicMessageChannel(
