...
class MXJSFlutterApp {
  ...
  MXJSStatefulWidget navigatorPushWithName(
      String widgetName, Key widgetKey, Map flutterPushParams, {String bizPath}) {
    MXJSLog.log(
        "MXJSFlutterApp:navigatorPushWithName: widgetName: $widgetName ");
    return createHostJSWidget(widgetName, widgetKey, flutterPushParams, bizPath: bizPath);
  }
  ...
  /// 查询对应widgetID的buildOwner
  MXBuildOwner queryElementBuildOwner(String widgetID) {
    MXBuildOwner buildOwner = _rootBuildOwnerNode.findChild(widgetID);
    return buildOwner;
  }
  ...
  dynamic callJS(MethodCall jsMethodCall) async {
    ...
    try {
      //  MXJSC-Support： v0.9.0 参数改为String
      // 与编译的JS交互统一用json字符串
      String jsArgsStr = json.encode(jsArgs);
      return await _appChannel.invokeMethod("callJS", jsArgsStr);
    } catch (e) {
      MXJSLog.error(
  ...
  dynamic callJSNeedFrequencyLimit(MethodCall jsMethodCall) async {
    ...
    _frequencyLimitMethodCallQueue.add(oneJsCallMap);
  ...
  void _refreshFrequencyLimitMethodCallQueue() {
    ...
    // MXJSC-Support： 参数改为String
    // 与编译的JS交互统一用json字符串
    String jsArgsStr = json.encode(jsArgs);
    _appChannel.invokeMethod("callJS", jsArgsStr);
