...
class MXJSStatefulWidget extends StatefulWidget with MXJSWidgetBase {
  ...
  /// JS文件业务路径，用于在jsAppPath后拼接，随后再拼接/name/bundle-name.js
  ...
  MXJSStatefulWidget.hostWidget(
      {Key key, this.name, this.parentBuildOwnerNode, Map flutterPushParams, this.bizPath})
  ...
  void refreshHostWidgetAfterJSEngineInit({bool jsEngineInitSuccess}) {
    if (jsEngineInitSuccess) {
      ...
      buildOwnerNode.callJSRefreshHostWidget(
          widget.name, widget.widgetID, context, widget.flutterPushParams, widget.bizPath);
    } else {
      // 直接刷新页面
      setState(() {});
    }
