...
class MXJSWidget extends StatelessWidget {
  ...
  final String bizPath;

  MXJSWidget({this.jsWidgetName, Key key, this.flutterPushParams, this.bizPath}) : this.key = key ?? UniqueKey();

  @override
  Widget build(BuildContext context) {
    MXJSLog.log("'MXJSPageWidget::build': MXJSFlutter.getInstance().navigatorPushWithName: $jsWidgetName key:$key");

    return MXJSFlutter.getInstance().navigatorPushWithName(jsWidgetName, key, flutterPushParams, bizPath: bizPath);
  }
}
...
class MXJSPageWidget extends MXJSWidget {
  MXJSPageWidget({String jsWidgetName, Key key, Map flutterPushParams, String bizPath})
      : super(jsWidgetName: jsWidgetName, key: key, flutterPushParams: flutterPushParams, bizPath: bizPath);
}
