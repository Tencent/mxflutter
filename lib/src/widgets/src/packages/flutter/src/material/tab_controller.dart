...
var _tabController = MXFunctionInvoke(
  ...
  ({
    ...
    if (_tabController.buildOwner.state is MXSingleTickerMixinWidgetState) {
      tickVsync =
          _tabController.buildOwner.state as MXSingleTickerMixinWidgetState;
    ...
    } else if (_tabController.buildOwner.state
        is MXSingleTickerAndKeepAliveMixinWidgetState) {
      tickVsync = _tabController.buildOwner.state
          as MXSingleTickerAndKeepAliveMixinWidgetState;
    } else if (_tabController.buildOwner.state
        is MXTickerAndKeepAliveMixinWidgetState) {
      tickVsync = _tabController.buildOwner.state
          as MXTickerAndKeepAliveMixinWidgetState;
    }
    ...
    if (listenerList != null && listenerList.length > 0) {
      tabController.addListener(_createListenerHandle(_tabController.buildOwner,
          mirrorID, "listenerCallback", tabController));
    }
...
VoidCallback _createListenerHandle(dynamic bo, String mirrorID,
    String functionName, TabController tabController) {
  VoidCallback cb = () {
    Map args = {
    ...
    bo.mirrorObjEventCallback(
        mirrorID: mirrorID, functionName: functionName, p: args);
  };
  return cb;
}
