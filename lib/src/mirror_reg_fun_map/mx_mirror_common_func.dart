...
Map<String, MXFunctionInvoke> registerMXJSStateWidgetSeries() {
  ...
  m[_tickerProviderInvoke.funName] = _tickerProviderInvoke;
...
/// 获取TickerProvider的逻辑
var _tickerProviderInvoke = MXFunctionInvoke(
  "TickerProvider",
      ({String elementID}) {
    var buildOwner = _tickerProviderInvoke.buildOwner;
    var app = MXJSFlutter.getInstance().currentApp;
    var buildOwnerByElement = app.queryElementBuildOwner(elementID);
    if (buildOwnerByElement != null) {
      buildOwner = buildOwnerByElement;
    }

    var tickVsync;
    if (buildOwner.state is MXSingleTickerMixinWidgetState) {
      tickVsync = buildOwner.state as MXSingleTickerMixinWidgetState;
    } else if (buildOwner.state is MXTickerMixinWidgetState) {
      tickVsync = buildOwner.state as MXTickerMixinWidgetState;
    } else if (buildOwner.state is MXSingleTickerAndKeepAliveMixinWidgetState) {
      tickVsync =
      buildOwner.state as MXSingleTickerAndKeepAliveMixinWidgetState;
    } else if (buildOwner.state is MXTickerAndKeepAliveMixinWidgetState) {
      tickVsync = buildOwner.state as MXTickerAndKeepAliveMixinWidgetState;
    }
